...
function [output] = GeneDriveABMfunc_091422(size0,tight,n_foci,kill_rad,swtch12_delay,iter)
...
nmax = round(1.2*n0); % maximum number of cells
...
gddepleted200 = false;   % used to check if 10 days have passed since gene drive population has been depleted (condition in while loop)
...
while n_curr>0 && n_curr<nmax && t_curr<tmax && ~gddepleted200

    % Engage switch 2 next iteration when condition met
    if (sum(phn_curr==2)+sum(phn_curr==3)) > n_swtch2 && ~swtch2
        swtch2 = true;
        t_swtch2 = t_curr;
...
%         state(:,:,s) = state_store;
        ...
        disp([tight kill_rad iter t_curr n_curr])
    end
