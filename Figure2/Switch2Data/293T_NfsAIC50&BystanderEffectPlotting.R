...
## Read in data
df = read.csv("293T_NfsA_CB1954IC50_051218Summary.csv",header=T)

## Plot IC50s

# Focus on 0% and 100% NfsA

sub.df = df[df$perc.NfsA%in%c(0,100),]
colnames(sub.df)[1] = "condition"
sub.df$dose = 10^sub.df$log.dose.nM
...
ggplot()+theme_bw()+
  geom_line(data=curves.df[curves.df$log.dose>=2,],aes(x=log.dose,y=pred,color=condition),size=2)+
  geom_errorbar(data=sub.df[sub.df$log.dose.nM>=2,],aes(x=log.dose.nM,ymin=mean-sem,ymax=mean+sem),color="black",width=0.15,size=1)+
  geom_point(data=sub.df[sub.df$log.dose.nM>=2,],aes(x=log.dose.nM,y=mean,fill=condition),size=4,color="black",shape=21)+
  scale_color_manual(values=c(WT.col,GD.col))+
  scale_fill_manual(values=c(WT.col,GD.col))+
  xlab("CB1954 Dose (nM)")+ylab("Relative Viability")+
  scale_y_continuous(breaks=c(0,0.5,1),limits=c(0,NA))+
  ggtitle("S2 vNfsA Dose Response")+
  theme(
...
# ggsave("293T_NfsAIC50.png",width=5,height=5)
...
sub.df = df[df$log.dose.nM==5,]

sub.df$killing = 1-sub.df$mean

m = (sub.df$killing[sub.df$perc.NfsA==100]-sub.df$killing[sub.df$perc.NfsA==0])/(100-0)
b = sub.df$killing[sub.df$perc.NfsA==0]

sub.df$no.bystndr = m*sub.df$perc.NfsA+b

ggplot(sub.df,aes(x=perc.NfsA,y=killing))+theme_bw()+
  ...
  xlab("Percent Expressing S2 vNfsA")+ylab("Relative Drug Effect")+
  ggtitle("S2 vNfsA Bystander Effect")+
  theme(plot.title=element_text(hjust=0.5,size=22,face="bold"),
        axis.title=element_text(size=20,face="bold"),
        axis.text = element_text(size=18,face="bold",color="black")
  )
# ggsave("293T_Cb1954Bystander.png",width=5,height=5)
