...
function output = SRGMouse_20230309(pop_treat,mut,frac_res,dim_res,a_T,a_B,b,d,npats,frac_gd,swtch_dly)
...
S_0 = round(pop_treat*(1-frac_res-frac_gd));
R_0 = round(pop_treat*frac_res);
G_0 = round(pop_treat*frac_gd);

pop_init = [S_0 R_0 0 0 G_0 0 0 0];
...
parfor r = 1:npats
    ...
    switch1 = true; % Begin with Switch 1
    switch2 = false;
    t_switch1end = NaN;
    Tsen = Tsen_vec1;
    Xsen = Xsen_vec0;
...
% 
% for j = 1:npop
%     semilogy(time,pop(:,j),'Color',colors(j,:),'LineWidth',2);
%     hold on
% end
% 
% title('Pop Dynamics')
